...
const [optimizedImages, setOptimizedImages] = useState<{ format: 
string; url: string }[]>([]); 
...
className="w-60 mt-2" /> 
      ...
      {optimizedImages.length > 0 && ( 
        <div className="mt-6"> 
          ...
          <table className="mt-4 border-collapse border border-gray
300 w-full text-center"> 
            <thead> 
              <tr className="bg-gray-100"> 
                <th className="border border-gray-300 p
2">Formato</th> 
...
2">{img.format}</td> 
                  <td className="border border-gray-300 p-2"> 
                    <img src={img.url} alt={img.format} className="w
60" /> 
                  </td> 
                </tr> 
              ))} 
